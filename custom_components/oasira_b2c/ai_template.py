"""Template functions for Oasira AI Conversation."""
...
from .ai_const import DEFAULT_WORKING_DIRECTORY, DOMAIN
...
async def async_setup_templates(hass: HomeAssistant) -> bool:
    """Set up template functions for Oasira AI Conversation."""
...
async def async_unload_templates(hass: HomeAssistant) -> bool:
    """Unload template functions for Oasira AI Conversation."""
...
class ExtendedOpenAITemplateManager:
    ...
    async def async_setup(self) -> None:
        ...
        _LOGGER.debug("Setting up Oasira AI Conversation template functions")

        # Register in existing environments
        if "template.environment" in self.hass.data:
            self.hass.data["template.environment"].globals[TEMPLATE_OASIRA_AI] = (
                self._oasira_ai
            )
        ...
        def template_environment_init(
            ...
            if hass:
                template_env_self.globals[TEMPLATE_OASIRA_AI] = (
                    self._oasira_ai
                )
    ...
    async def async_on_unload(self) -> None:
        ...
        _LOGGER.debug("Tearing down Oasira AI Conversation template functions")
